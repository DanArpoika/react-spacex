...
export default class Launch extends React.Component {
  ...
  static async getInitialProps({ query }) {
    try {
      const call = await axios.get(
        `https://api.spacexdata.com/v2/launches?flight_number=${query.launch}`,
      );
      const { data } = call;
  ...
  render() {
    ...
    data = data[0]; // eslint-disable-line prefer-destructuring
    ...
    return (
      <main>
        <Head>
          <title>
            Flight {flightNumber} - {date} | SpaceX Launch Data
          </title>
        </Head>
        <Container>
          <CellSection>
              ...
              {data.details && (
                <CellRow>
                  <Cell cols={8}>
                    ...
                    <p style={{ lineHeight: 1.5, margin: '0.75em 0 0 0', fontSize: '0.875rem' }}>
                      {data.details}
                    </p>
                  </Cell>
                  <Cell cols={4} border="left">
          ...
          <CellSection>
            ...
            <CellGroup>
              {data.rocket.first_stage.cores.map(core => (
                <CellRow key={encodeURI(core.core_serial)}>
                  <Cell cols={4}>
          ...
          <CellSection marginTop="4rem">
            ...
            <CellGroup>
              {data.rocket.second_stage.payloads.map(load => (
                <CellRow key={encodeURI(load.payload_id)} templateRows={2}>
                  <Cell cols={4} rows={2}>
